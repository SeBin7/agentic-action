...
- Codex 실행은 기본 `codex exec --json --ephemeral` 경로를 사용하고, `agent_message` 이벤트를 결과 본문으로 사용

## 출력 파일
성공:
- `bridge/done/YYYYMMDDTHHMMSSZ_<thread>_<taskid>_from_codex.result.md`

실패:
- `bridge/error/YYYYMMDDTHHMMSSZ_<thread>_<taskid>_from_codex.error.md`

## 결과 메타
성공(`result`):
- `exit_code`, `elapsed_ms`, `retries`, `status: done`
